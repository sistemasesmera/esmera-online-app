...
import { ExternalLink, FileText, Image, Paperclip, Trash2, Upload, X } from "lucide-react";
import { useRef, useState, useTransition } from "react";
import { toast } from "sonner";

import {
  deleteEnrollmentAttachment,
  saveEnrollmentAttachment,
} from "@/app/(app)/enrollments/[id]/attachments-actions";
...
export function EnrollmentAttachmentsTab({
  enrollmentId,
  studentId,
  attachments: initial,
  canUpload,
  ...
  function handleConfirmUpload() {
    ...
    startUpload(async () => {
      ...
      const filePath = `enrollments/${enrollmentId}/${crypto.randomUUID()}.${ext}`;
      ...
      const result = await saveEnrollmentAttachment(
        enrollmentId,
        studentId,
        filePath,
        file.name,
  ...
  function handleDelete(attachment: AttachmentWithUrl) {
    ...
    startUpload(async () => {
      const result = await deleteEnrollmentAttachment(
        attachment.id,
