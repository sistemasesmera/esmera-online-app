...
} from "@/app/(app)/students/[id]/attachments-actions";
...
export function AttachmentsTab({
  ...
  // Pending file waiting for title confirmation
  ...
  function handleConfirmUpload() {
    ...
    startUpload(async () => {
      ...
      const filePath = certificateId
        ? `students/${studentId}/certificates/${certificateId}/${uniqueName}`
        : `students/${studentId}/${uniqueName}`;
  ...
  function handleDelete(attachment: AttachmentWithUrl) {
    ...
    startUpload(async () => {
      const result = await deleteAttachment(attachment.id, attachment.file_path, studentId);
      if (result.error) {
        toast.error(result.error);
      } else {
